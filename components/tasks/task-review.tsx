...
interface TaskReviewProps {
  ...
  onConfirm: (subtasks: SubtaskData[], overallWeight: EffortWeight, expectedOutput: string) => void;
...
export function TaskReview({
  ...
  const [subtasks, setSubtasks] = useState<SubtaskData[]>(initialBreakdown.subtasks);
  const [overallWeight, setOverallWeight] = useState<EffortWeight>(initialBreakdown.suggestedOverallWeight);
  const [expectedOutput, setExpectedOutput] = useState(initialBreakdown.suggestedExpectedOutput);
  ...
  const handleSendMessage = async (message: string) => {
    ...
    setChatMessages(prev => [...prev, { role: 'ai', content: 'Updated your task breakdown!' }]);
  };
      ...
      <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <Button variant="outline" size="sm" onClick={handleRegenerate} disabled={isModifying}>
              🔄 Regenerate
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Be specific and observable. What would someone see if they looked at your work?
            </p>
          </div>
